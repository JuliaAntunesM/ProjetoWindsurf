// Funcionalidade do Quiz de Exercícios na Cadeira
...
function toggleCheckbox(option) {
    ...
    if (!Array.isArray(userAnswers['question' + questionNumber])) {
        userAnswers['question' + questionNumber] = [];
    }
    
    // Adiciona ou remove a opção do array de respostas
...
async function loadQuizData() {
    try {
        const response = await fetch('chair-quiz-data.json');
...
// Adiciona elementos flutuantes de cadeira ao fundo
function addFloatingChairs() {
    const floatingChairs = document.createElement('div');
    floatingChairs.className = 'floating-chairs';
    
    // Adiciona 12 cadeiras flutuantes
    for (let i = 1; i <= 12; i++) {
        const chair = document.createElement('div');
        chair.className = `floating-chair chair-${i}`;
        floatingChairs.appendChild(chair);
    }
    
    document.body.appendChild(floatingChairs);
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Adiciona elementos flutuantes de cadeira
    addFloatingChairs(); // Adicionando as cadeiras flutuantes
